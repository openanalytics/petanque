#' Draw the field.
#' @param newPlot Logical, if TRUE (FALSE by default),
#' should a blank plot be created?
#' @return No return value, a plot is drawn to the current device.
#' @importFrom oaPlots blankPlot
#' @importFrom RColorBrewer brewer.pal
#' @importFrom graphics polygon text segments rect
...
#' Draw a flower.
#' @param x Numeric with flower position
#' @param stemHeight Numeric with stem height.
#' @param colNum Numeric, number of the flower.
#' @inherit drawField return
#' @importFrom graphics points
#' @importFrom RColorBrewer brewer.pal
#' @importFrom oaColors oaColors
...
drawHuman <- function(color = "orange") {
	...
	points(x = -0.4, y = 0.4, cex = 3, pch = 19) # head
...
#' Plot the animation for a specific ball throw.
#' @param distance Numeric with distance at which the ball is thrown.
#' @param color String with color.
#' @param step Numeric with step.
#' step = Inf preserves 'usual' animation, otherwise 'shiny' animation is used
#' @inherit drawField return
#' @importFrom graphics points
...
#' Plot an animation for the collisions between balls.
#' @param posDF Data.frame with ball positions.
#' @inheritParams animateThrow
#' @inherit drawField return
    ...
    for (jStep in seq_len(min(4, stepsInCycle))) {
      ...
      if (animationStep >= realStep)
        points(x = flying$x + xProp[jStep] * flying$travelDist,
            y = rep(yPos[jStep]/(iCycle+1), nFlying), cex = 1, pch = 19, col = oaColors(flying$color)) 
...
#' 'Refresh' the plot for a petanque game.
#' @inheritParams animateCollision
#' @inheritParams drawField
#' @inherit animateCollision return 
#' @importFrom oaColors oaColors
...
#' Redraw the balls based on the updated positions.
#' @param except Numeric with balls ID to not re-redraw.
#' @inheritParams animateCollision
#' @inherit animateCollision return 
