...
#							" has won with ", score(), paste0(" point", if (score() > 1) "s." else "."))
          msg <- tagList("Game finished! ", br(), 
              actionLink("gotoRankings", tagList("See Rankings", icon("arrow-right"))), 
              "Press", actionLink("enter", "Enter"), "to start a new game!")
        } else {
...
#				req(nDistr(), animationFinished(), gameActive())
  ...
  output$gameArea <- renderUI({
        ...
        } else HTML("&nbsp;") # FIXME: hack
        tagList(
            div(style = "color:darkgray;", h3(msg)),
            uiOutput("game")
        )
      })
...
#                      intermediateData(posDF)
                    ...
                    posDF$travelDist <- 0 # allow overlapping balls
                    finished <- TRUE
                  }
...
#                refreshPlot(posDF, newPlot = FALSE)
  ...
  observeEvent(input$enter, {
        if (animationFinished()) { # prevent pressing again while ball is flying
          if (!gameActive()) {
            ...
            chosenDistr(NULL) # to make sure change event is triggered
            chosenDistr(picked)
            # change the player
...
#' Open default svg device.
#' @param ... Any parameters for \code{\link[svglite]{svgstring}} function.
#' @return No return value, the svg device is opened.
