...
maxTarget <- 10
allRes <- matrix(NA, 3000000, 11)
for (iRep in 1:3000000){
	...
	allRes[iRep, 5] <- rweibull(1, maxTarget * runif(1,0.5,2.5), maxTarget * runif(1,0.2,1.5))
	...
	allRes[iRep, 8] <- rnorm(1, maxTarget * runif(1, 0.4, 0.6), runif(1, 0.5, 3))
	...
	allRes[iRep, 11] <- runif(1, min = maxTarget*0.05, max = maxTarget*1.5)
}
...
allDiffs <- abs(allRes[,-1] - allRes[,1])

hist(apply(allDiffs, 1, min), main = "Absolute minimum distance among all 10 distributions")
tab2display <- table(apply(allDiffs, 1, which.min))/nrow(allDiffs)
names(tab2display) <- c("binomial", "Poisson", "uniform", "Weibull", 
		"log-normal", "chi2", "normal", "Bernouli", "beta", "degenerate")[as.numeric(names(tab2display))]
